...
# TestP2PConn: A peer we use to send messages to matildad, and store responses.
...
class CompactBlocksTest(MatildaTestFramework):
    ...
    # This test actually causes matildad to (reasonably!) disconnect us, so do this last.
    ...
    # Test that matildad requests compact blocks when we announce new blocks
    ...
    def test_incorrect_blocktxn_response(self, test_node):
        ...
        # Note that it's possible for matildad to be smart enough to know we're
    ...
    def test_getblocktxn_handler(self, test_node):
        version = test_node.cmpct_version
        node = self.nodes[0]
        # matildad will not send blocktxn responses for blocks whose height is
