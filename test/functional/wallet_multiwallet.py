...
Verify that a matildad node can load multiple wallet files
"""
...
class MultiWalletTest(MatildaTestFramework):
    ...
    def run_test(self):
        ...
        if self.options.descriptors:
            exp_stderr = r"Error: SQLiteDatabase: Unable to obtain an exclusive lock on the database, is it being used by another matildad?"
        else:
        ...
        threads = []
        for _ in range(3):
            n = node.cli if self.options.usecli else get_rpc_proxy(node.url, 1, timeout=60, coveragedir=node.coverage_dir)
        ...
        if self.options.descriptors:
            assert_raises_rpc_error(-4, "Wallet file verification failed. SQLiteDatabase: Unable to obtain an exclusive lock on the database, is it being used by another matildad?", self.nodes[0].loadwallet, wallet_names[0])
        else:
