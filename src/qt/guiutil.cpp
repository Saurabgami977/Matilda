...
#include <qt/matildaaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Matilda address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new MatildaAddressEntryValidator(parent));
    widget->setCheckValidator(new MatildaAddressCheckValidator(parent));
}

bool parseMatildaURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no matilda: URI
    if(!uri.isValid() || uri.scheme() != QString("matilda"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!MatildaUnits::parse(MatildaUnits::MTL, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseMatildaURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseMatildaURI(uriInstance, out);
}

QString formatMatildaURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("matilda:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(MatildaUnits::format(MatildaUnits::MTL, info.amount, false, MatildaUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openMatildaConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", MATILDA_CONF_FILENAME));
    ...
    /* Open matilda.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Matilda.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Matilda (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Matilda (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for Matilda*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "matilda.desktop";
    return GetAutostartDir() / strprintf("matilda-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a matilda.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Matilda\n";
        else
            optionFile << strprintf("Name=Matilda (%s)\n", chain);
