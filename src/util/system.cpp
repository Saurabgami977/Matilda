...
const char * const MATILDA_CONF_FILENAME = "matilda.conf";
const char * const MATILDA_SETTINGS_FILENAME = "settings.json";
 ...
 * https://github.com/matilda/matilda/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //matilda-tx using stdin
        std::string val;
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp) const
{
    ...
    if (filepath) {
        std::string settings = GetArg("-settings", MATILDA_SETTINGS_FILENAME);
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Matilda
    // macOS: ~/Library/Application Support/Matilda
    // Unix-like: ~/.matilda
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Matilda";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Matilda";
#else
    // Unix-like
    return pathRet / ".matilda";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", MATILDA_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure Matilda Core copyright is not removed by accident
    if (copyright_devs.find("Matilda Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Matilda Core developers";
    }
