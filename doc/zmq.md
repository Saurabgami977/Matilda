...
The Matilda Core daemon can be configured to act as a trusted "border
router", implementing the matilda wire protocol and relay, making
...
The ZeroMQ feature in Matilda Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building matildad:
...
For instance:
    ...
    $ matildad -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/matildad.tx.raw \
...
These options can also be provided in matilda.conf.
...
From the perspective of matildad, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into matildad directly. Furthermore, no information is
...
using. Matildad appends an up-counting sequence number to each
